...
if [[ -f /usr/share/zsh/plugin-managers/zinit/zinit.zsh ]]; then
    source /usr/share/zsh/plugin-managers/zinit/zinit.zsh
...
if [[ -n "$SSH_CLIENT" ]]; then
    PS1='$(f_user) $(f_hostname) $(f_colored_path) $(f_arrow)'
else
...
# use C-r to search through command history
# bindkey "^R" history-incremental-search-backward

# Make cursor a line when in insert mode, and a block when in normal mode
cursor_beam() {
    echo -ne "\e]50;CursorShape=1\x7"
}

cursor_block() {
    echo -ne "\e]50;CursorShape=0\x7"
}
